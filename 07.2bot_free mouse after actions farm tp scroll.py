...
class Cord:
    ...
    lookforchest = (590,390,747,402)#(590,386,747,402)cord for comparison if there is a chest, and if it is normal or rare chest.
    lookforitem = (640,330,697,390)
    #normalchest cords
    n_chest = (434,256)
    n_open_chest = (396, 118)
...
def look_for_chest():
    ...
    return a
def look_for_item():
    box = (Cord.lookforitem)
    im = ImageOps.grayscale(ImageGrab.grab(box))
    a = array(im.getcolors())
    a = a.sum()
    print 'look_for_item=',a
    #im.save(os.getcwd() + '\\lookforitem__' + str(int(time.time())) + '.png', 'PNG')    
    return a    
...
def startGame():
    ...
    if normalchest == look_for_chest():
        ...
        time.sleep(5.0)
        #check if there is tp scroll on opened normal chest
        if tpscroll == look_for_item():
            print 'Nice! It is a tp scroll!'
            time.sleep(5.0)
            grabitem(Cord.char1)
        ...
        if tpscroll == look_for_item():
            ...
            errormessage() #to stop the program
        time.sleep(5.0)
    ...
    if rarechest == look_for_chest():
        ...
        if rarechest == look_for_chest():
            ...
            leftClick(Cord.r_use_tokens)#after failing lockpick atempt, the "leave chest" button comes to position 3 on clickable choices
            time.sleep(5.0)
    ...
    if negatetokens == look_for_chest():
            leftClick(Cord.no) #just to be sure we'll not spend tokens accidentaly, instead of leaving chest.
            time.sleep(5.0)
            print 'Negating the use of tokens, after accidentaly clicking on unlock chest with tokens.'
    ...
    if openedrarechest == look_for_chest():
        print ' Oh boy, nevermind. Everybody is full. Human intervention needed here.'
#check if the chest was opened
    if openedrarechest2 == look_for_chest():
        grabitem(Cord.char1)
        print 'Trying to drag item to character 1.'
    if openedrarechest2 == look_for_chest():
        grabitem(Cord.char2)
