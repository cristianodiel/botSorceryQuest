...
def startGame():
    ...
    if rarechest == look_for_chest():
        ...
        if rarechest == look_for_chest():
            mousePos(Cord.none)#r_use_tokens) #after failing lockpick atempt, the "leave chest" button comes to position 3 on clickable choices
            leftClick()
            time.sleep(5.0)
